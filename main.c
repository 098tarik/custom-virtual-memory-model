/*
    ...
    --------------------------------
    This program simulates a virtual memory system using a page table and swap disk.
    It manages memory access, page faults, and implements multiple page replacement algorithms:
    1. First-In, First-Out (FIFO)
    2. Random Replacement (RAND)
    3. Custom Replacement (Prefetching)

    The program uses a simulated disk to swap pages in and out of "physical" memory.
*/
...
// check_frame_availability: Scans the frame table to find a free physical frame.
// Returns the frame index if one is free, or -1 if memory is full.
...
// custom_replacement_policy: A basic prefetching algorithm.
// When page P is faulted in, this policy attempts to also load page P+1 if:
// - P+1 exists and is not already in memory
// - The system has more than 1 frame
// This helps exploit spatial locality in memory access patterns.
...
// replace_page: Implements the page eviction logic.
// Selects a victim page based on the current policy (FIFO or RAND).
// If the victim page is "dirty" (PROT_WRITE is set), it is written back to disk before eviction.
...
// page_fault_handler: Called by the OS (via signals) when a page fault occurs.
// It determines whether to allocate a new frame or evict an existing one.
// It uses replace_page() when memory is full.
...
// Configures the simulation parameters (pages, frames, algorithm), sets up the 
// virtual disk and page table, and executes the specified test program.
